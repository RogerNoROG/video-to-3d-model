...
### 7.9 定稿：色度清异物 + 低密度过滤 + 盒裁，三道一起上

上面 7.8 的版本留下的问题是**沿棱一圈粗糙"屑"**。后来把这三种手段拆开单独试，
才发现它们各自解决不同问题、而且**互相冲突**：

| `density_quantile` | 顶面（低密度碎屑区） | 侧面棱边（低密度折痕） |
| --- | --- | --- |
| 0.005 | 干净 ✓ | **被啃烂** ✗ |
| 0 | **弹坑** ✗ | 完好 ✓ |

根因：**棱和凹槽内壁本来就是低密度高曲率区**，密度过滤在削掉顶面碎屑的同时也把棱啃掉了。
所以正确做法是**分工**——顶面碎屑交给颜色判据去，密度过滤只留很小的值：

**第 1 步：按颜色剔掉外来几何（`tools/clean_cube_points.py`）**

判据是**归一化色度** `(R−B)/max(R,G,B)`：榉木 ≈ 0.25（偏粉），灰白台面 ≈ 0.07。
除以 max 是为了与亮度无关 —— 阴影里的木头 R−B 绝对值也小，但归一化色度依然高，
所以不会误删阴影（实测侧面木色中位 0.249）。

```bash
./.venv/bin/python tools/clean_cube_points.py \
  storage/e7f9f9e5…/merge/merged_final_c1_cube.ply \
  storage/e7f9f9e5…/merge/merged_cube_v3.ply \
  --band 0.15 --threshold 0.20
```

实测：带内删 2.06%（全云）。`--band 0.04` 时凹槽边缘会留一圈奶白，带深 0.15 才清干净。

**第 2 步：Poisson + 盒裁 + 补平（`tools/finish_mesh.py`）**

```bash
./.venv/bin/python tools/finish_mesh.py \
  --input  storage/e7f9f9e5…/merge/merged_cube_v3.ply \
  --output storage/e7f9f9e5…/merge/merged_cube_final.glb \
  --depth 9 --density-quantile 0.0015 --clip-tol 0.006 \
  --smooth-iterations 8 --max-hole-edges 1500
```

`--clip-tol`（本次新增）：顶点伸出「点云 0.5%/99.5% 分位包围盒」超过该容差的三角形直接删掉，
切出的小口由补洞步骤补平。**这一步是清掉棱上毛屑的关键**——物体已知是立方体时，
两段扫描在棱/角处各留一层表面，Poisson 沿棱起皱长出的毛屑大多伸到盒外，按盒裁掉最省事。

**实测效果**（29.0 M 点云，depth 9）：

| 版本 | 盒裁容差 | 删三角面 | 补洞后边界边 | 观感 |
| --- | --- | --- | --- | --- |
| v3 无盒裁（M1/N1） | — | — | 788 | 棱上仍有毛屑 |
| **Q1（定稿）** | **0.006** | 53,817 | **2（几乎全封闭）** | **棱角干净笔直，凹槽/圆孔完整** |
| Q4 | 0.0045 | 90,151 | 0 | 与 Q1 相当，角上碎屑略多 |
| Q3 | 0.003 | 143,477 | 2,077 | 裁过头，切口太大补不平 |

→ 定稿：**`merged_cube_final.glb`（135 MB）**

**试过但更差、别再走的路子**（生成物已删，仅留结论）：

- **体素预降噪**（`--pre-voxel 0.003 / 0.004 / 0.006`，无论配 depth 8 还是 9）→ 异物被糊成更大的整块；配深度 8 时 Poisson 直接出"融化的蜡"。**想靠体素把两层表面合层是行不通的**
- **`density_quantile=0` 全关过滤**（即使已做色度清理）→ 顶面又出现台面弹坑
- **重度 Taubin**（15 / 30 次）→ 毛屑只是变圆润，不会消失；30 次以上木纹开始糊
- **点云级连通分量清异物** → 只能删掉 0.80%，异物与主体是连通的
- **靠亮度找碎屑** → 不可靠：上半部暗点中位高度 −279 mm，说明暗点主要是**竖面的阴影**，不是碎屑

**仍然存在的一处瑕疵**：凹槽末端与左侧面交汇的那个角上仍有一小丛深色碎屑。
该处点云本身就是多层乱结构（要根治只能重拍）。

### 7.10 「棱上的深色接缝」：盒夹取代盒裁（定稿，2026-09-13）

7.9 定稿后沿棱还剩一条深色接缝，肉眼像是"两段扫描叠层留下的沟"。**高倍放大才看出真相**：
那是一排**补上的洞**，而且每个补片的圆心顶点用的是**全网格平均色** → 渲染成深灰圆盘，
连起来就成了"缝"，放大能看清放射状的 spokes。

**两个独立的坑，都要修：**

1. **补洞顶点的颜色**（`tools/finish_mesh.py::fill_boundary_holes`）
   - 错：`np.tile(colors.mean(axis=0), ...)` —— 全网格平均色
   - 对：**该洞边界一圈**的平均色 `colors[members].mean(axis=0)`

2. **洞是哪来的** —— 不是密度过滤，是**盒裁**
   - 实测：密度归零后 Poisson 原始输出边界边仅 **106**；**盒裁**（删盒外三角形）一下切出
     **9,856 条边 / 264 个开口**，补洞再从圆心拉出放射状平面扇 → 沿棱一圈"缝"
   - **改法：盒夹（`--snap-tol`）** —— 不删三角形，而是把盒外的顶点**夹回盒子表面**：
     凸出部分被压平贴到面上，**不产生开口**（边界边仍是 106），补片与面天然齐平
   - `--clip-tol`（盒裁）保留但**别再用**；`--snap-tol` 与 `--clip-tol` 互斥，snap 优先

**实测对比**（`merged_cube_v3.ply`，depth 9，density 0，Taubin 8~12）：

| 版本 | 盒外顶点 | 裁/夹后边界边 | 开口 | 观感 |
| --- | --- | --- | --- | --- |
| 盒裁 0.006（旧定稿） | 22,492 | **11,068** | 264 | 沿棱一圈放射状灰色补片 |
| **盒夹 0.006（新定稿）** | 24,320 | **106** | 2 | **棱上干净，无补片** |
| 盒夹 0.004（更紧） | 43,557 | 106 | 2 | 左上角被压出一大片放射面，**反而更差** |

→ 定稿：**`merged_cube_final.glb`（136 MB）**，配套 **`merged_cube_light.glb`（24 MB，depth 8）**

```bash
./.venv/bin/python tools/finish_mesh.py \
  --input  storage/e7f9f9e5…/merge/merged_cube_v3.ply \
  --output storage/e7f9f9e5…/merge/merged_cube_final.glb \
  --depth 9 --density-quantile 0 --snap-tol 0.006 \
  --smooth-iterations 12 --max-hole-edges 1500
```
...
- 判断网格质量**别用 matplotlib 散点图**；`<model-viewer>` 里把相机拉近（`camera-orbit` 半径调小）
  才看得出"接缝"到底是什么 —— 这次就是拉到 2.1 m 才看清是补片而不是沟
