...
| 方案 | 成本 | 说明 |
| --- | --- | --- |
| A. 对 `sparse/1` 也跑一遍稠密重建 | **7 小时** | 得到含底面的第二个模型。设 `MODEL_API_PATCH_MATCH_GEOM_CONSISTENCY=false` 减半到 ~3.5h；再加 `MODEL_API_PATCH_MATCH_MAX_IMAGE_SIZE=960` 可压到 ~1h |
| B. 重拍一段 | 十几分钟 | 物体**不要拿起来**，一次连续环绕拍完所有面，就不会被拆成两段 |

⚠️ 两段**没有共享图像**，COLMAP 的 `model_merger` 用不了（它需要重叠图像）。要合成单一"带底面的完整模型"，需在对 `sparse/1` 跑完稠密后按**物体自身几何**做双扫描配准合并 —— **该功能目前未实现**。
