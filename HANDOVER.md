...
**问题**：Poisson 在法线朝向不一致 / 点密度不均处会长出**不封闭**的表面。实测合并模型（3990 万三角面）留下 **520 个开口、33,698 条边界边**，渲染出来就是主体上的撕裂缺口。

**做法**（三步，顺序不能颠倒）：

1. **Poisson**（`point_cloud_to_glb`）
2. **补洞**：找只被 1 个三角形使用的边 → 按顶点连通性分成一个个开口 → 每个开口加一个中心顶点、用扇形三角面封上。
   - ⚠️ 补的面必须**反向遍历共用边**（原三角形用 a→b，补的面必须用 b→a），否则法线会翻
   - ⚠️ **补完必须重算顶点法线**。只动拓扑不算法线会让整块模型明暗全错（实测过一次，整块变深色沙粒）
   - `--max-hole-edges N` 可只补小洞；实测大开口留空反而更糟（露出白色内壁），**默认全补**
3. **Taubin 平滑**（默认 3 次）：抹掉重建噪声；顶点颜色不参与平滑，木纹颜色纹理不受影响

**实测效果**（30 M 点合并点云，包围盒 1.49）：

| 阶段 | 顶点 | 三角面 | 边界边 | GLB |
| --- | --- | --- | --- | --- |
| 深度 10 + 补洞 + 平滑 | 19.8 M | 39.9 M | 33,698 → **4** | 966 MB |
| **深度 9 + 补洞 + 平滑（推荐）** | **2.05 M** | **4.10 M** | 3,380 → **0** | **100 MB** |
